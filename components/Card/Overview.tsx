...
export const Overview: FC<Props> = ({
  ...
  function getTypeOfSpan(currentLimit: number, defaultWordLimit: number) {
    return currentLimit === defaultWordLimit ? (
      <span
        className={styles.overview__more}
        onClick={() => setCurrentWordLimit(fullTextLength)}
      >
        {" "}
        ...more
      </span>
      ...
      <span
        className={styles.overview__more}
        onClick={() => setCurrentWordLimit(defaultWordLimit)}
      >
        {" "}
        less
      </span>
  ...
  return (
    <p className={styles.container}>
      {wordsInOverview
        .filter((ele, wordCount) => wordCount < currentWordLimit)
        .join(" ")}
